			...
			if (value < 10 && prob((10 - value) * 1.5))
				for (var/mob/living/carbon/human/H in viewers(2, holder.owner))
					if (H != holder.owner && prob(30 - value) * 2)
						...
						H.vomit()
						H.visible_message("<span class='alert'>[H] throws up all over [himself_or_herself(H)]. Gross!</span>")
				...
				holder.owner.visible_message("<span class='alert'>[holder.owner] throws up all over [himself_or_herself(holder.owner)]. Gross!</span>")
				holder.owner.vomit()
